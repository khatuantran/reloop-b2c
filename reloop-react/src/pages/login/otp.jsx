...
export default function Otp() {
  ...
  return (
    <>
      <AuthShell flow="login" steps={steps} activeStep={2}>
          <div className="grid grid-cols-[1fr_400px] gap-space-32">
            <div className="bg-bg-elevated rounded-[28px] shadow-clay border border-border-subtle p-space-48">
              <a href="/login" className="inline-flex items-center gap-space-4 font-mono-md text-[11px] text-text-tertiary hover:text-tier-s font-semibold mb-space-16">
                <span className="material-symbols-rounded !text-[14px]">arrow_back</span>
                Quay lại bước 1
              </a>
              <span className="font-mono-md text-[11px] uppercase tracking-[0.2em] text-tier-s font-bold mb-space-8 block">BƯỚC 2/2 · XÁC THỰC OTP</span>
              ...
              <p className="font-body-md text-[14px] text-text-secondary mb-space-32">
                ...
                · <a href="/login" className="text-tier-s font-bold hover:underline">Đổi số</a>
              </p>
      
              {/* OTP cells */}
              <div className="grid grid-cols-6 gap-space-12 mb-space-16 max-w-[440px]">
                {[2, 6, 0, '', '', ''].map((d, i) => (
                  <div className={cx(['aspect-square rounded-2xl shadow-clay-sm flex items-center justify-center font-display font-extrabold text-[36px] tabular-nums transition', d !== '' ? 'bg-clay-mint border-2 border-tier-s text-tier-s' : i === 3 ? 'bg-bg-surface border-2 border-tier-s shadow-inset-soft text-text-primary animate-pulse' : 'bg-bg-surface border-2 border-border-subtle text-text-tertiary'])}>
                    {d !== '' ? d : i === 3 ? '|' : ''}
                  </div>
                ))}
              </div>
              ...
              <div className="flex items-center justify-between mb-space-32 px-space-4 max-w-[440px]">
                <span className="font-mono-md text-[12px] text-text-tertiary inline-flex items-center gap-space-4">
                  ...
                  Còn <strong className="text-amber-deep tabular-nums">04:23</strong> trước khi mã hết hạn
                </span>
                <button className="font-mono-md text-[12px] text-tier-s font-bold hover:underline">Gửi lại OTP</button>
              </div>
      
              <a href="/" className="w-full bg-text-primary text-white py-space-16 rounded-2xl font-semibold shadow-clay text-[15px] inline-flex items-center justify-center gap-space-8 hover:-translate-y-[2px] transition mb-space-16">
              ...
              <div className="bg-clay-mint/40 rounded-2xl border border-tier-s/20 p-space-16 flex items-start gap-space-12">
                ...
                <p className="font-body-md text-[12px] text-text-secondary leading-relaxed">
                  <strong className="text-text-primary">Không nhận được SMS?</strong> Kiểm tra sóng → Bấm "Gửi lại" → Hoặc thử <a href="#" className="text-tier-s font-bold hover:underline">đăng nhập qua Zalo OAuth</a>
                </p>
              </div>
            </div>
