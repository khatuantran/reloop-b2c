...
export default function Success() {
  ...
  const broadcastTimeline = [
    { time: 'Vừa xong', label: 'Đơn đã tạo', sub: 'Mã RL-2026-001241', icon: 'check_circle', state: 'done' },
    { time: 'Đang diễn ra', label: 'Hub Q.7 nhận đơn', sub: '4 collector trực hôm nay', icon: 'broadcast_on_personal', state: 'active' },
  ...
  const quickActions = [
    { icon: 'visibility', label: 'Xem chi tiết đơn', href: `/orders/${order.id}/index.html`, accent: 'amber-deep', bg: 'bg-clay-butter', border: 'border-amber-deep/30' },
    ...
    { icon: 'chat', label: 'Chat Anh Hùng', href: '#', accent: 'info', bg: 'bg-clay-sky', border: 'border-info/30' },
  ];
  return (
    <>
      <main className="pt-[100px] pb-space-96 min-h-screen">
          <div className="max-w-[1280px] mx-auto px-[80px]">
            {/* Confetti hero */}
            <section className="grad-hero rounded-[32px] shadow-clay-lg border-2 border-amber-deep/30 p-space-48 mb-space-32 relative overflow-hidden">
              {/* Decorative confetti */}
              <svg className="absolute top-space-24 left-space-32 w-8 h-8" viewBox="0 0 32 32" fill="none">
                <path d="M16 4l2 6h6l-5 4 2 6-5-4-5 4 2-6-5-4h6z" fill="#F2D58F" opacity="0.7" />
              </svg>
              ...
              <svg className="absolute bottom-space-24 right-space-48 w-10 h-10" viewBox="0 0 40 40" fill="none">
                <rect x="8" y="8" width="6" height="20" fill="#E8B340" opacity="0.5" transform="rotate(15 11 18)" />
                <rect x="22" y="6" width="6" height="22" fill="#52E08D" opacity="0.5" transform="rotate(-20 25 17)" />
              </svg>
              ...
              <div className="relative grid grid-cols-[auto_1fr_auto] gap-space-32 items-center">
                ...
                <div className="text-col">
                  <span className="font-mono-md text-[12px] uppercase tracking-wider text-tier-s font-bold flex items-center gap-space-8">
                    ...
                    SPEED MODE · ĐƠN ĐÃ ĐẶT
                  </span>
                  <h1 className="font-h1 text-h1 text-text-primary mt-space-12">Hub Q.7 đã nhận đơn của bạn</h1>
                  <p className="font-body-lg text-[15px] text-text-secondary mt-space-12">Mã đơn <strong className="font-mono-md text-amber-deep tabular-nums">{order.id}</strong> · tạo lúc {order.createdAt}. {order.collector} sẽ gọi xác nhận trong 5 phút.</p>
                  <div className="mt-space-20 flex flex-wrap gap-space-12">
                    <span className="px-space-16 py-space-8 rounded-full bg-bg-elevated/95 backdrop-blur shadow-clay-sm flex items-center gap-space-8">
                      ...
                      <span className="font-mono-md text-[12px] text-text-primary font-bold tabular-nums">{order.floor.toLocaleString('vi-VN')} — {(order.floor + order.premiumMax).toLocaleString('vi-VN')}đ</span>
                    </span>
                    <span className="px-space-16 py-space-8 rounded-full bg-bg-elevated/95 backdrop-blur shadow-clay-sm flex items-center gap-space-8">
                      ...
                      {broadcastTimeline.map((t) => (
                        <div className="relative flex items-start gap-space-20">
                          <div className={cx(['relative z-10 w-14 h-14 rounded-2xl flex items-center justify-center shadow-clay-sm shrink-0 border-2',
                            ...
                            t.state === 'active' ? 'bg-clay-butter text-amber-deep border-amber-deep animate-pulse' :
                          ...
                          <div className="flex-1 text-col pt-space-4">
                            <div className="flex items-center gap-space-12 flex-wrap">
                              ...
                              <span className={cx(['px-space-8 py-[2px] rounded-full font-mono-md text-[10px] font-bold uppercase tracking-wider whitespace-nowrap',
                                ...
                                t.state === 'active' ? 'bg-amber-deep/15 text-amber-deep' :
                  ...
                  <div className="grid grid-cols-2 gap-space-16">
                    <div className="bg-clay-butter/40 rounded-2xl p-space-20 border border-amber-deep/20">
                      <div className="flex items-center gap-space-12 mb-space-12">
                        <span className="material-symbols-rounded fill !text-[20px] text-amber-deep">kitchen</span>
                        <span className="font-mono-md text-[10px] text-amber-deep uppercase tracking-wider font-bold">VẬT PHẨM</span>
                      </div>
                      <span className="font-display font-bold text-[14px] text-text-primary block text-col">{order.item}</span>
                      <span className="font-mono-md text-[11px] text-text-tertiary block mt-space-4">{order.weight} kg · còn hoạt động</span>
                    </div>
                    ...
                    <div className="bg-clay-mint/40 rounded-2xl p-space-20 border border-tier-s/20">
                      ...
                      <span className="font-display font-extrabold text-[20px] text-text-primary tabular-nums block">{order.floor.toLocaleString('vi-VN')}đ</span>
                      <span className="font-mono-md text-[11px] text-tier-s font-bold mt-space-4 block tabular-nums">+ tối đa {order.premiumMax.toLocaleString('vi-VN')}đ premium</span>
                    </div>
                    ...
                    <div className="bg-clay-lavender/40 rounded-2xl p-space-20 border border-info/20">
                      ...
                      <span className="font-mono-md text-[11px] text-text-tertiary block mt-space-4">Tự động sau khi cân xong</span>
                    </div>
                  </div>
                </section>
                ...
                <section className="bg-clay-mint rounded-[28px] shadow-clay-sm border-2 border-tier-s/30 p-space-32">
                  ...
                  <div className="grid grid-cols-3 gap-space-16">
                    <div className="bg-bg-elevated/70 rounded-2xl p-space-20 text-col">
                      ...
                      <p className="font-body-md text-[12px] text-text-secondary mt-space-4">Ngắt nguồn 24h trước để hết gas lạnh</p>
                    </div>
                    <div className="bg-bg-elevated/70 rounded-2xl p-space-20 text-col">
                      <div className="w-10 h-10 rounded-2xl bg-bg-elevated flex items-center justify-center shadow-clay-sm mb-space-12">
                        <span className="material-symbols-rounded fill !text-[20px] text-tier-s">cleaning_services</span>
                      </div>
                      <span className="font-display font-bold text-[13px] text-text-primary block">Lau khô bên trong</span>
                      <p className="font-body-md text-[12px] text-text-secondary mt-space-4">Đỡ mất điểm spot check premium</p>
                    </div>
                    <div className="bg-bg-elevated/70 rounded-2xl p-space-20 text-col">
                      <div className="w-10 h-10 rounded-2xl bg-bg-elevated flex items-center justify-center shadow-clay-sm mb-space-12">
                      ...
                      <p className="font-body-md text-[12px] text-text-secondary mt-space-4">Đo cửa + thang máy trước, tránh kẹt</p>
                    </div>
                  </div>
                </section>
              </div>
              ...
              <div className="flex flex-col gap-space-16">
                ...
                <div className="bg-bg-elevated rounded-[24px] shadow-clay border border-border-subtle p-space-24">
                  ...
                  <div className="mt-space-16 flex items-center gap-space-12">
                    <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-clay-butter to-amber-deep flex items-center justify-center text-white font-display font-extrabold text-[22px] shadow-clay-sm shrink-0">
                      H
                    </div>
                  ...
                  <div className="mt-space-12 grid grid-cols-2 gap-space-8">
                    ...
                    <div className="bg-bg-base/60 rounded-xl p-space-12 text-col">
                      <span className="font-mono-md text-[9px] text-text-tertiary uppercase tracking-wider font-semibold block">Sẽ gọi</span>
                      <span className="font-display font-bold text-[14px] text-tier-s mt-space-4 block">~5 phút</span>
                    </div>
                  </div>
                  <a href="#" className="mt-space-12 w-full bg-amber-deep text-white py-space-12 rounded-2xl font-mono-md text-[12px] font-bold shadow-clay-sm inline-flex items-center justify-center gap-space-8 hover:-translate-y-[1px] transition">
                ...
                <div className="bg-clay-blush/40 rounded-[24px] shadow-clay-sm border border-tier-h/20 p-space-24">
                  ...
                  <p className="font-body-md text-[12px] text-text-secondary mt-space-12 text-col">Huỷ miễn phí trước slot 2 giờ. Đổi slot/địa chỉ trước 1 giờ.</p>
                ...
                <div className="bg-clay-mint/50 rounded-[24px] shadow-clay-sm border border-tier-s/20 p-space-24">
                  ...
                  <p className="font-body-md text-[12px] text-text-secondary mt-space-12 text-col">Đơn này được lưu thành <strong>template Tủ lạnh</strong>. Lần sau bán đồ tương tự, chỉ cần 2 tap.</p>
