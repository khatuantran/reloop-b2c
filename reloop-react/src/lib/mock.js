...
//
// Shapes (informal — no TypeScript anymore):
//   Tier:        'S' | 'B' | 'C' | 'H'
//   OrderStatus: 'COMPLETED' | 'PENDING_VERIFY' | 'IN_TRANSIT' | 'AT_PICKUP' | 'CANCELLED'
//   User:    { id, name, phone, email, address, district, trustScore, greenPoints, memberSince }
//   Wallet:  { zaloPayBalance, holdAmount }
//   Collector: { id, name, tier: 1|2|3, vehicle, plate, rating, completedOrders, phone, avatarColor }
//   Order:   { id, tier, status, item, itemSlug, mode?, winningBid?, speedFloor?, hazardousItems?,
//              weight, amount?, advance?, settled?, expectedRange?, createdAt, completedAt?,
//              collectorId?, bomReal?: { Cu, Fe, Al, plastic }, massBalance? }
//   Transaction: { id, type: 'IN'|'OUT'|'HOLD'|'RELEASE', amount, date, orderId?, description }
//   Price:   { material, perKg, unit }
//   BomItem: { id, name, weightRange:[n,n], defaultWeight, expectedBom, realBom, expectedRange:[n,n] }
...
export function trustScoreZone(score) {
